...
const buildMinuteSnapshotBars = (service: ServiceDetail, minutes: number): TimelineBar[] => {
  ...
  return Array.from({ length: minutes }, (_, index) => {
    ...
    const status: TimelineBarStatus = incidents.some((incident) => incidentOverlapsWindow(incident, minuteStartMs, minuteEndMs))
      ? 'down'
      : 'up'
...
const buildHourlyOverlapBars = (service: ServiceDetail, hours: number, hoursPerBar = 1): TimelineBar[] => {
  const referenceMs = getTimelineReferenceMs()
  const currentHourStartMs = Math.floor(referenceMs / HOUR_IN_MS) * HOUR_IN_MS
  const incidents = timelineIncidentsByServiceId.value.get(service.id) ?? []
  const barCount = Math.ceil(hours / hoursPerBar)
  const barDurationMs = hoursPerBar * HOUR_IN_MS

  if (!service.runtime.lastCheckAt) {
    return Array.from({ length: barCount }, (_, index) => {
      const hourStartMs = currentHourStartMs - (barCount - 1 - index) * barDurationMs
      const hourEndMs = hourStartMs + barDurationMs
...
const serviceCheckBars = (service: ServiceDetail) => buildHourlyOverlapBars(service, 20)
...
const recentTimelineBars = computed(() => {
  return activeService.value ? buildHourlyOverlapBars(activeService.value, 48, 2) : []
})
...
const uptime24hStat = computed(() => {
  return buildUptimeStat(t('dashboard.console.last48Hours'), 2)
})
              ...
              <section class="service-health-grid" :aria-label="$t('dashboard.console.serviceHealthLabel')">
                ...
                <article class="health-card health-card--top shell-card">
                  <div class="health-card__header">
                    <span class="health-card__label">{{ $t('dashboard.console.last48Hours') }}</span>
